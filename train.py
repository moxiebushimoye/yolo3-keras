...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = 'model_data/apple_loss8.66_weights.h5'
    #------------------------------------------------------#
    ...
    Init_Epoch          = 0
    Freeze_Epoch        = 50
    Freeze_batch_size   = 4 ##显存较大的情况下，可以设置的更大
    #Freeze_lr=1e-3
    #------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    ### 如果显存较大，batchsize可以大一点，如果显存较小，batchsize就设置小一点###
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = 300
    Unfreeze_batch_size = 2 #由于占用显存较大，所以设置的batch_size 会稍微小一点
    # UnFreeze_lr = 1e-4
    #------------------------------------------------------------------#
    #   Freeze_Train    是否进行冻结训练
    #                   默认先冻结主干训练后解冻训练。
    #                   如果不想进行冻结训练，就将其设置为False
    #------------------------------------------------------------------#
    ...
    if True:
        ...
        ### TensorBoard 可视化训练参数，设置 write_grads=True可视化梯度下降过程（默认False),设置True必须先设置histogram_freq=1（默认0）
